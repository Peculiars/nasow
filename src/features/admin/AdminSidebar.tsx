...
const AdminSidebar = () => {
  ...
  const menuItems = [
    ...
    { title: "Executives", href: "/admin/executives", badge: null, iconPath: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" },
    { title: "Nasowite of Week", href: "/admin/nasowites-of-the-week", badge: null, iconPath: "M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" },
  ...
  return (
    <div className="font-inter">
      ...
      >
        {isMobileOpen ? <X className="size-5 text-white"/> : <Menu className="size-5 text-white"/>}
      </button>
      {isMobileOpen && (
      ...
      <aside className={`fixed top-0 left-0 h-screen bg-white border-r-2 border-gray-200 transition-all duration-300 z-40 ${isSidebarOpen ? "w-64" : "w-20"} ${isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 border-b-2 border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="hidden lg:block p-1.5 hover:bg-gray-100 rounded-lg">
                {isSidebarOpen ? <ChevronLeft className="size-5 text-gray-700" /> : <Menu className="size-5 text-gray-700" />}
              </button>
            </div>
          </div>
          ...
          <div className="p-4 border-t-2 border-gray-200 space-y-1">
            <button onClick={() => router.push("/admin/settings")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${pathname === "/admin/settings" ? "bg-[#9179E0] text-white" : "text-gray-700 hover:bg-gray-100"}`}>
